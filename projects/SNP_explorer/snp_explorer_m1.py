...
# the input is a SNP ID and windows for genes / probes. ie.  python snp_explorer_m1.py rs9629043 200000 200000
# it ouputs three tsv files with predetermined names:
#	1. file containing ensembl ID, gene ID and distance from gene TSS to SNP
#	2. file containing probe ID and distance from probe to SNP
#	3. file containing peak ID and distance from peak to SNP
...
filename1 = "/Users/james/Desktop/jtopham_prj/data/rosmap/snp_annotation/annotated_SNPs_filtered.tsv"           # maps SNP ids to genomic position (filtered from dbSNP list)
...
with open(filename1, "r") as ins:    			                      
	for line in ins:
		if (line.split()[4] == this_snp):
			snp_position = int(line.split()[2])
			snp_chr = line.split()[1]
			break

if snp_position == -1:
	print 'Error ' + '(' + this_snp + '): unable to map SNP to position'
	...
	for line in ins:
		if line.split()[2] == snp_chr:
			if int(line.split()[3]) >= g_min_range and int(line.split()[3]) <= g_max_range:
				this_gene = "ENSG" + line.split()[0] + '\t' + line.split()[1] + '\t' + str(abs(snp_position - int(line.split()[3])))
				nearby_genes.append(this_gene)

if len(nearby_genes) == 0:
	print 'Error ' + '(' + this_snp + '): no genes found within designated window'
	...
	for line in ins:
		if line.split()[1] == snp_chr:
			if int(line.split()[2]) >= m_min_range and int(line.split()[2]) <= m_max_range:
				this_probe = line.split()[0] + '\t' + str(abs(snp_position - int(line.split()[2])))
				nearby_probes.append(this_probe)

if len(nearby_probes) == 0:
	print 'Error ' + '(' + this_snp + '): no methylation probes found within designated window'
	...
	for line in ins:
		if line.split()[3] == snp_chr:
			...
			if this_mean >= a_min_range and this_mean <= a_max_range:                       # find acetylation peaks within chosen window of SNP
				this_peak = line.split()[0] + '\t' + str(abs(snp_position - this_mean))
				nearby_peaks.append(this_peak)

if len(nearby_peaks) == 0:
	print 'Error ' + '(' + this_snp + '): no acetylation peaks found within 1mb window'
